...
an interactive map-based 'planning support system' (PSS)
[@Geertman2009]. As with previously documented online PSS, the NPCT
provides a range of information to the user to enable evidence-based policy
[@Pettit2013]. The NPCT differs from previous work on PSS, however, in its focus
on sustainable transport, provision of data at the 'flow' and transport network
level and provision of open source code and open-access data to allow the
model to be reconstructed by others. The model is implemented in R, a mature language for
statistical computing. All of the code has been made available
under the MIT license on the GitHub platform.^[See [github.com/npct/pct](https://github.com/npct/pct).] The NPCT is,
to the authors' knowledge,
the first online PSS to be developed that is focused on cycling.
...
route network. Scenarios can be based on nationa, regional or city-wide targets
...
and related infrastructure is a critical problem in transport planning which
must be solved for cycling levels to soar [@Pucher2010].
...
By aggregating
data from the flow level to geographic zones the model also enables areal-based
planning (encouraging users to focus on particular zones rather than flows).
...
under various future scenarios. Thus the model differs from previous approaches
to modelling cycling potential.  @Parkin2008 used a similar regression-model
approach at the areal level to predict the current level of cycling.
In addition, local survey data identifying the origin of
'potentially cyclable trips' but the travel corridors they are likely to take
[@TfL2010].

It is important to note that, as with the previously mentioned models,
the NPCT approach does not include the effects
of specific policies, such as a particular piece of new
infrastructure (although further work could explore adding such
capability), on the local level of cycling. The purpose of such 'meso scale'
transport models in the context of active travel
is primarily for strategic planning [@Moniruzzaman2012].
Instead, the models allow  estimation of where new cycling
trips are most likely to be generated given specific overall increases in
cycling, such as a target level of cycling (typically measured as a
proportion of all trips) being met.
The 'Cycling Delivery Plan' scenario, for example, assumes a doubling in the
level of cycling,
this being the proposed target of the Government’s draft
...
if cycling increases overall by this amount nationally or regionally,
