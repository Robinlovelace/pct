...
- Anna Goodman (London School of Health and Tropical Medicine)
...
explains how the PCT's open source approach allows it to be
deployed in new cities and countries. We conclude that the method can help
plan for modal shift to active travel, which can in turn improve citizen health and
...
would have to depend on motorised modes or other people [@Aldred2008].
...
The PCT provides a method to answer the question:
...
'desire line'. This is a straight line connecting the origin
...
enabling the outcome to be displayed rapidly; calculating
...
on the route network assuming a *circuity* [@Iacono2010] value of 1.3.
...
Hilliness was included as a continuous variable in our model
...
the *godutch* scenario focuses on long-term potential.
The 'Go Dutch' scenario models what would happen if
English people were as likely to cycle for a given trip (i.e. of the same
distance and hilliness) as Dutch people, by applying Dutch
...
the predicted levels of cycle use in *godutch*
...
easier to achieve growth in cycle use in places where cycling is
already common.
...
between *govtarget* (which represents only a doubling nationwide) and *godutch*
