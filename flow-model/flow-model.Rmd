...
Emerging evidence shows that high-quality and appropriate infrastructure
...
the Netherlands; and 'Ebikes', an exploratory analysis of increasing the distance
...
developed methods, such as Aecom's proprietary Permeability Assessment Tool,
...
cycle trips plausibly occur? As highlighted in the discussion,
...
An individual-level regression model described by @Zhang2014 provided
a basis for prioritising investment in cycle paths at the city-level in Brazil.
By allocating the results to the route network the authors were able to
prioritise routes to "achieve maximum impacts early on" [@Zhang2014]. Our model performs a
...
The scenarios are highly responsive to user input because the
scenarios are pre-calculated, rather than being computed 'on the fly',
To ensure the model is accessible, the
user is not required to specify any numerical model parameters to interact with
the PCT. Advanced users can modify any part of the model, however.
The PCT's open source licence reduces common barriers to
development of additional features in the PCT: cost and access to source code.
Anyone with programming
skills can modify the code base for their own needs.
The PCT is therefore part of a wider trend in transport research towards
greater transparency in software development and collaboration [@Tamminga2012;@novosel2015agent].. 
The transferability of the PCT to new settings is likewise enhanced by its use of aggregate OD data,
...
a relatively small proportion of OD pairs. For example, in the City of
Manchester (whose area encompasses the centre of Greater Manchester but
also extends several miles southwards),
disregarding OD pairs with fewer than 30 commuters leaves
...
another way to reduce the number of flow-lines is to set the maximum Euclidean
(or "crow-flies") distance. This can be done in conjunction with setting a maximum
number of commuters.
...
In essence, the PCT provides a model of of how cycle use in different locations is
inhibited by various constraints, and the potential for increased cycle use if these
contraints were reduced or removed.  Although not explicitly represented in the model,
these constraints can be assumed to include a mix of infrastructural and cultural
constrains (i.e. the deterrent effects of sub-optimal cycling conditions and the
cultural resistance of certain groups to take up cycling --- these may well be
inter-related).  They also include the factors of distance and hilliness which have
already been discussed.

For the case study presented in this paper, four scenarios were developed in order to
allow transport planners to consider different possibilities for the future of cycling
in England (n.b. the *Ebike* scenario has yet to be fully implemented). 
The following sections explain these four scenarios.  However it is worth summarising them at the outset, as this illustrates the flexibility of the PCT as a tool that could be to develop other scenarios for different geographical and political contexts.

- Government target (*govtarget*). This scenario models a doubling of
the number of cycling trip-stages in England. 
...
with above-average increases (i.e. more than a doubling) in areas where
there is a significant proportion of short (i.e. potentially cyclable)
commute trips and low levels of current cycle use.
Conversely, areas with higher cycle use and/or a low proportion
of short commutes will have below-average growth in relative
(but not absolute) terms.
...
- Gender Equality (*gendereq*). This scenario illustrates the increase in cycling that would result if women were as likely to cycle in England as men – or, more specifically, if the proportion of female cycle commuters in each flow were the same as the current proportion of males. In places where cycling is the norm, women are equally as likely to cycle as are men (Pucher et al 2010).
The *gendereq* scenario therefore represents
the elimination of one specific cultural constraint,
namely the lower propensity of women than men
to cycle. This scenario is expected to have greatest impact where
cycling conditions are hostile.
...
in cycle use that would would be achieved through the widespread uptake of electric
cycles ('Ebikes').
Ebikes enable longer journeys, as well as making cycling a viable option for older
people and people with impaired mobility who might otherwise be unwilling to cycle
at all. This scenario therefore goes beyond the *dutch*
scenario in also reducing the
impacts of the distance and hilliness constraints to cycling.
...
rate of cycling tend to see the greatest increase in cycling under the *govtarget* scenario.
...
Under the 'national doubling' assumption of the *govtarget* scenario, this
...
a 9.6% proportion of cycle commuters in the *govtarget* scenario.
...
rate of cycling of 35% ($pcycle(govtarget)$ = 35%). Likewise, a desire line of
...
This implementation of the *govtarget* is in line with findings from
...
social and cultural disparities in cycling. Under *govtarget*,  areas with
...
This scenario was to provide an indication of the increased rate of cycling
that may be possible due the electric cycles.
This is the most ambitious and also the most speculative
of the scenarios presented in this paper and it
...
This will build on analysis of the influence of Ebikes on propensity to cycle
in the Netherlands that is being undertaken in paralell to the work
...
allocation of investment. Specifically, transport planners said that the tool
could be useful for setting local targets or implementing locally-specific policies within
a single city or region. To follow-up on such feedback, modifications of the methods
...
options.^[Targets have proliferated in recent years, including an official
...
through increased physical activity [@Fraser2011] and the local energy and
carbon savings of cycling uptake
[@Lovelace2011c].
...
AG’s contribution was also supported by a National Institute for Health Research (NIHR) post-doctoral fellowship. The views reported in this paper are those of the authors and do not necessarily represent those of [the DfT], the NIHR, the NHS or the Department for Health.
