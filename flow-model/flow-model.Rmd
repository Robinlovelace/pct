...
a trip begins and the destination where it ends.
Flows therefore represent 'desire lines' --- the straight lines connecting
places where people want to travel [@Chan2003] ---
...
In practice a variety of data sets may work best when combined.
Individual and areal data, for example, can be combined for transport
planning purposes using spatial microsimulation [@Lovelace2014-jtg].
Even when travel survey data contains geographical information,
sample sizes are generally comparatively small, less than 1% of the population.
This limits the use of travel surveys for geographical modelling. 
With uptake of mobile telephones by populations worldwide, flows
...
inferring modal splits from travel surveys is a feasible alternative
...
Transport decisions (such as where to build new cycle routes)
are often made at the local level [@Gaffron2003]. In England this
...
From the National Travel Survey, only 1.1% of
cycle commutes were found to exceed this distance.
...
Future research could explore the impacts of
...
travel demand patterns on the potential rate of cycling in
different areas.
...
The distance decay
curve displayed in Fig. 4, based on equation x, was used.
...
Future work should refine the treatment of hilliness in the PCT.
...
# UK;^[The mean proportion of male cycle commuters to work from the 2011 census
...
inter-group differences in the rate of cycling fall
as the rate of cycling increases. In the Netherlands,
cycling is almost equally common among males, females, and
people of different socio-economic status, although ethnic
persist [@Fishman2015]. Thus for
...
The Gender Equality scenario (*gendereq*) builds
on the *observed level of cycling* (*OLC*) from the 2011 Census.
...
In the PCT can therefore shed light on whether cycling investment should prioritise
areas of relatively low current propensity (but high potential) or relatively
high current propensity
(but lower potential) [@Sloman2014;@Aldred2014].
The answer is likely to depend on the timescale of interest.
...
"is there a strong link between a country's cycling potential and actual
rates of cycling?"
...
A more specific limitation of the PCT method is its current lack of inclusion of
cycle route quality. Future versions of the tool could make use of
