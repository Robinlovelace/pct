...
- Route-allocation of OD lines, enabling specific routes to be identified
for improvement.
...
license. The OD model underlying the PCT was written in R,
...
represented by the $T_{ij}$ matrix notation in transport modelling [@...].
 'desire lines', the straight lines connecting
...
the rate of cycling could be at the OD and route level under various future scenarios.
...
The model underlying the PCT is based on aggregate OD data
...
which is gradually gaining traction in academic transport research [@Tamminga2012;@novosel2015agent]
...
# can be translated into ODs at high geographical resolution and allocated to the
...
The OD level model described in this paper can work for
...
# t1 <- xtable(head(flow[c(1:3, 12)]), caption = "Sample of the 'OD' input dataset, representing the number of people who commute from locations within and between administrative zones (MSOAs)", label = "tbl:flow")
...
```{r, echo=FALSE, fig.cap="Illustration of 'OD data' converted into geographical lines between origin and destination pairs for Coventry. Width represents the total number of trips. Note the use of population-weighted (as opposed to geographic) centroids used for the point of departure and destination."}
# data("flowlines")
...
are the same, indicating *intra-zone* movement.
...
- Route distance, assigned to each OD pair
...
We used the English Census 2011 dataset on travel to work data
...
A variety of emerging sources can also provide data on travel OD pairs,
...
a relatively small proportion of OD pairs. In Manchester,
...
(*dutch*) scenarios is illustrated in Fig. 7. Note that the top 20 OD pairs
in Norwich under *cdp* assumptions are dominated by the current rate of cycling,
with the highest OD pairs projected to continue to be found towards the east of
...
are more representative of short-distance routes across the city overall.
...
different OD pairs could shift in the future were
