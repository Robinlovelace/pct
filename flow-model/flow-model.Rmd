...
The Propensity to Cycle Tool (PCT) seeks to inform such decisions by providing
evidence-based support tools that model current and potential future
...
license. The flow model underlying the PCT was written in R,
...
These typically connect residential origins with workplace, educational or other
...
- *Flow* (or *origin-destination*) data relating the number of trips
taking place between administrative zones.
...
the flow and geographic datasets together, *zone ids* are needed in both
...
each flow in both directions. These were labelled flow-lines after @Tobler1987;
their handling built on published work on cartographic visualisation [@Rae2009;@Wood2010].
Such datasets are available in many countries and cities across the world.
...
- Flow data representing the number of trips between origin destination pairs,
...
- Route distance, assigned to each flowline
...
We used the English Census 2011 dataset on travel to work flows
...
A variety of emerging sources can also provide data on travel flows,
including 'Big Data' from commercial companies.
These alternative souces of flow data include:
...
script, flow model and output visualisations were run on a region-by-region basis.
...
is the bottle-neck in terms of computational time.

To reduce data processing and visualisation times
only a sub-sample of flow-lines for each region were used.
The aim was to reduce the number of flows whilst retaining the overall
travel pattern. We did this using a minimum values for the total flow,
following the insight that the
distribution of number of commuters per flow is
skewed: commuters using a few major 'commuter corridors'
mean a high proportion of displacements can be represented by
a relatively small proportion of flow-lines. In in the city of Manchester,
...
to estimate the potential rate of cycling at the flow level.
...
the proportion of trips made by cycling per flow ($pcycle$).
The concept of 'distance decay' [@Martinez2013]
was used in the model to describe the (non-linear) relationship
between the route distance of flow-lines and the proportion of trips made by cycling.
...
As described earlier, Cyclestreets.net was used to allocate flow-lines to
