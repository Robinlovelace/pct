---
title:
  "The National Propensity to Cycle Tool: An open source online system for sustainable transport planning"
output: 
  word_document: 
    fig_caption: yes
...
is an effective way of promoting cycling. To maximize the benefits of new
infrastructure, it is important to build in the right places.
Cycle paths, for example, will have the greatest impact if they built
along 'desire lines' of greatest latent demand.
The National Propensity to Cycle Tool (NPCT)
has been developed to inform such decisions at the policy development stage.
It does this by providing an 
evidence-based support tool for the assessment of current and potential
future distributions in cycling across cities and regions. This paper
describes the development of the model underlying the NPCT and
the application of this model to three cities in England.
Origin-destination 'flow' data, combined with
quantitative information about administrative zones,
form the basis of the model, which estimates cycling potential as a function of
route distance, hilliness and other factors at the flow and area level.
Multiple scenarios were generated and interactively displayed, including
'Government target', to double of cycling and a more ambitious
‘Go Dutch’ scenario for envisioning a future in which people in England
are as likely to cycle a given trip as people in the Netherlands.
The model can be accessed at [geo8.webarch.net/master/](http://geo8.webarch.net/master/).
This paper also explains how the model's open source code base
allows the NPCT approach to be deployed in new contexts.
We conclude that the method presented has the potential to assist
with planning for cycling-dominated cities worldwide, which can help
...
cycling can also empower people who would otherwise be totally dependent on motorised
modes and other people with an efficient means of self propulsion [@Aldred2008].
Mounting evidence of the external costs car-dominated
transport systems [@Newman1999;@Han2008;@Mizutani2011;@Shergold2012]
and the benefits of active travel [@Oja2011;@Saunders2013;@DeNazelle2011]
has pushed cycling up the
transport agenda in many cities, regions and nations worldwide.
...
is a critically important yet seldom raised
issue. The design [@Heath2006;@TfL2014;@WG2014] and geographic location [@Aultman-Hall1997;@Minikel2012]
of cycle paths are
important factors influencing the attractiveness of cycling, which
is crucial for high cycling uptake [@Pucher2010].
The premise of this paper is that
planning support tools, such as the National Propensity to Cycle Tool
(NPCT) described in this paper, can help the associated investment to be
spent effectively.

The National Propensity to Cycle Tool (NPCT) is
an interactive map-based 'planning support system'
[@Geertman2009]. As with such previously documented online systems, the NPCT
provides a range of information to the user to enable evidence-based policy
[@Pettit2013]. The NPCT differs from previous work, however:
in the method's focus on future scenarios of active travel;
in its estimation and visualisation of information at three levels
(at the zone, 'desire line' and transport network level);
and in its use of open source code and open access data.
The open approach to model development is key to the NPCT utility to others,
enabling it to be recreated by others in new contexts. The NPCT is, to our knowledge,
the first planning support system of any kind to focus explicitly on cycling.
The policy relevance of the NPCT stems from its ability to
be used to develop, compare and explore various scenarios
of cycling futures. As well as a transport-focussed planning
support system, the NPCT is also a meso scale transport model (explained in
the next paragraph): it simulates and visualises local and route level
increases in cycling in response to achieving national,
regional or city-wide targets. 

The NPCT is a *strategic* transport planning tool. Unlike 'microscopic' transport
models such as SUMO and PTV VISSIM, which simulate vehicular traffic in
real time [@Behrisch2014;@Krajzewicz2014], the NPCT is scalable
to the national level.
Nor is the NPCT a 'macroscopic' transport model [e.g. @Kolosz2013]
due to its estimation of
cycling potential at relatively high (and flexible) levels of geographic resolution.
Instead, the NPCT operates at the 'meso scale', an appropriate
level active travel planning [@Moniruzzaman2012]. The model does not treat time
as a continuous variable, instead providing 'snapshots' of future scenarios [@McCollum2009].

Unlike @McCollum2009 and other national-level scenario-based approaches,
the NPCT allows estimation of where new cycling trips are most likely to
...
Thus the NPCT provides a method to answer the question:
...
This ability to model propensity to at the flow-level is one important,
and novel, feature of the NPCT.
...
typically connecting residential origins with workplace, educational or other
destinations. As described below in more detail, the model takes the current
rate of cycling and other variables per flow and uses this to simulate what
the rate of cycling per flow could be under various future scenarios.
By aggregating data from the flow level to geographic zones, the NPCT
model also enables areal-based planning (encouraging users to focus on
particular zones rather than flows). This inclusion of flow-level data is one
important difference between NPCT and previous approaches to modeling cycling
potential. @Parkin2008, for example, used
a multiple regression model to simulate cycling at
...
The model underlying the NPCT is based on flow data
...
the approach is also highly flexible and relatively easy to modify. The NPCT
can thus be used basis for creating new cycling planning tools in diverse contexts
and relying on multiple data inputs.

```{r, echo=FALSE}
# Key to the NPCT's utility as a PSS for strategic sustainable transport planning
# is its ability to visualise various scenarios of cycling futures. The
# flow-level model described in the Methods section explains how these scenarios
# can be translated into flows at high geographical resolution and allocated to the
...
# Due to the open approach in model development the NPCT methodology can be
# replicated in many other locations worldwide. We therefore expect this article
# to be useful not only academically, in terms of development of a flow-level
...
# The NPCT was funded by the UK's Department
...
# The NPCT model presented in this paper was deployed for three
# 'pilot' cities which
...
operates 'under the hood' in the NPCT online interface.
This means that the
...
interface more accessible, and the pre-calculation of outputs
also means that the interface is fast and responsive.
To ensure the model is accessible, the
user is not required to perform any model parameters to interact with
the NPCT. Advanced users are free to analyse and
and modify the NPCT's underlying code base, however,
through the project's online code repository.
...
in future versions of the NPCT.
This could involve running training sessions for transport planners with
programming experience.

All of the code underlying NPCT has been made available open source under the
...
The remainder of this paper therefore describes the model underlying the NPCT,
...
We used flow data disaggregated by mode for England although,
by estimating parameters from other data,
it would also be possible to estimate cycling potential in
cases where no break-down by mode is possible.
...
data quality. Official datasets from which flows can be derived
are available for commuting trips and in some cases education trips in
many countries.
...
With the rapid update of mobile telephones by populations worldwide, flows
inferred from device-cell communications are particularly promising. Recent
...
Although mobile phone data does not provide information of mode of travel,
inferring modal splits from smaller local surveys is a feasible alternative
that would allow tools such as the NPCT to estimate where future demand is greatest.
In any case, flow data are
provided as a 'flow matrix' (with rows representing origins
...
At a bare minimum, this means the centroids of each zone in the study area, preferably weighted by population and/or work location.
In addition to this, geographic variables could also include additional
...
Tables 1 and 2 and Fig. 1 illustrate the two input datasets along with the
...
Table: Sample of the 'flow' input dataset, representing the number of people who commute from locations within and between administrative zones (MSOAs)
...
text file
of flows between unique origin destination pairs, disaggregated by mode (see Table 2).
Note that this is a square table, which was loaded as a `data.frame`
...
end in the same zone, as is the case in the top row of Table 1.
...
administrative zones. We used the official 'Medium Super
...
Future work will explore using small geographic units as the basis
of the model. In England, for example,
Lower Super Output Areas (LSOAs, average population of around 1,600) could
be used as origins and Workplace Zones (WZ) could form the
destinations.[See [ons.gov.uk/ons/guide-method/geography/products/census/lookup/index.html](http://www.ons.gov.uk/ons/guide-method/geography/products/census/lookup/index.html) for further details.]
Using different
geographic units for the origins and destinations has two major advantages:
the centroids will be better-matched
...
Therefore for pragmatic reasons we decided to divide-up England into
the regions at which
...
is to create buffers around the the regions (as discussed below).
...
We recommend using the geographical regions at which money is spent on
transport as the regional level of analysis for future versions
of the NPCT.
...
The route allocation algorithm, implemented 'in the
cloud' by CycleStreets.net (an online routing service for
planning cycle trips),
is the bottle-neck at present in terms of time required to run the model
for a given region at present.  In order to reduce the time to add new regions
only a sub-sample of flow-lines for each region were used.
...
flows that are used by a minimum number of commuters. The
distribution of number of commuters per flow is highly
skewed by high numbers of commuters using a few major 'commuter corridors',
therefore a high proportion of commuters can be represented by
a relatively small proportion of flow-lines. In Manchester,
for example, disregarding flow-lines with fewer than 30 commuters leaves
15% of all possible origin destination pairs but includes almost 70% of commuters.
The number 30 is a parameter that
...
flow-lines, and this can be done in conjunction with setting a maximum number of commuters.
We used a maximum value of 15km Euclidean distance which roughly translates to 20km
...
This is because we have represented the flows-lines from zone centroid to zone centroid
due to how the data was structured.
...
to show the underlying model to the users; to ensure transparency;
and to give further information to users.
...
The output tab also communicates the results of the model,
...
scenarios of change. Illustrations of how the number of cyclists using
different flow-lines could shift in the future were
described as 'very useful' for transport planning during user testing.
In addition, the visualisation of different scenarios
enables 'visioning', an activity that has great potential
...
to the model to improve model fit. We also included hilliness as an
independent variable as this will impact on cycling rates.
The formula chosen to model the data with was:

$$
pcycle = \alpha + \beta_1 d + \beta_2 d^{0.5} + \theta H,
$$

where $d$ is distance (km, route distance) and
$H$ is the hilliness (described below) per flow.
$\alpha$ represents the intercept (the rate of cycling very short
trips), $\beta_1$ and $\beta_2$ represent the rate of distance decay
...
to account for the aforementioned exponential decay.^[See [github.com/npct/pct/blob/master/models/aggregate-model.R.](https://github.com/npct/pct/blob/master/models/aggregate-model.R).]
...
importance (in the UK context), the factors implemented in NPCT were:
...
We started with a simple approach to demonstrate the importance of
...
Additional explanatory variables can be included to predict
...
As explained above, additional variables could easily added.
...
expected given trip distances and hilliness we model an increase in
cycling. The larger the increase in cycling the less the baseline matters
...
@Sloman2014 that suggest building in areas with a high current rate
of cycling will further increase the rate of cycling. The method
also links to the discussion of 'propensity' vs 'potential' to cycle:
the former relates to short-term responses, based on
demographic, social and cultural likelihood of cycling. Some groups
are tempted to cycle, whereas others are averse to cycling
(this latter group was found in the baseline data for the
CityConnect project in deprived areas between Leeds and Bradford).
Under our implementation of the *cdp* scenario, these 'low propensity'
areas would still have a lower than expected rate of cycling based
on national trends. In the longer-term 'Go Dutch' scenario,
it is simply the potential rate of cycling that determines the
rate of flow, meaning that the current rate of cycling has
no bearing on the estimates.

The next scenario to be discussed is *Gender Equality*. In this
scenario cycling tends to grow much more in areas that already
have a high rate of cycling. Thus this scenario is more about
propensity than the long-term potential levels of cycling
described in *Go Dutch* and *Ebike* scenarios. In the long-run,
inter-group differences in the rate of cycling tend to zero
as the rate of cycling increases. In Holland, for example,
cycling is almost equally common among males, females, and
people of different socio-economic status. Thus for
strategic planning purposes demographics should play less of a
roll in influencing the decision-making process than it should
in implementing short-term 'quick-win' policies.
...
In future work we plan to refine this by also factoring in average differences in hilliness levels between England and the Netherlands
...
be possible in this model. In future work we will implement the
model fully by basing the changes to the distance decay
parameters on real data from the Dutch National Travel Survey.
Analysis of the influence of Ebikes on propensity to cycle
(of people using both using
ebikes and conventional bicycles) bikes in the Netherlands has
was conducted as part of the Co-benefits model. 
# Results
...
with the highest flows projected to continue to be found towards the east of
...
the pattern of cycling flows shifts substantially to the west.
The cycling pattern under the *dutch* scenario
are more representative of short-distance flows across the city overall.
...
The flexibility of the NPCT methodology
...
possible to use the NPCT as a platform for further research and planning
...
new scenarios of the futures and new features such as 'freeze scope'.
...
cycling targets, which have proliferated in recent years.^[For example, an official
...
translate national targets into national aspirations. With the NPCT tool,
local transport planners can answer the question, "can cycling here be expected
to surpass the national target of x% of trips by bike?"

Potential applications go
beyond these policy relevant near-tern applications which involved
re-running slightly modified versions of the model for different localities.
Wider extensions of the model that could build on the framework presented in this
paper include:

- Deployment of the NPCT for entire countries.
This would depend on having appropriate flow data. As indicated
...
- The extension of the model to cover different demographic
groups and breakdown by currently dominant mode
to enable more targeted cycling policies.
...
The NPCT's open source licence allows others to
modify it for their own needs, for example to implement these extensions and
others yet to be discovered. We actively encourage practitioners
...
the proportion cycling by a certain year and which
...
to the model could also include using the NPCT methodology to
enable public engagement in the strategic planning process around active travel.

The wide range of opportunities to extend the tool do not mean that the NPCT
methodology can solve every urban mobility problem. Transport policy is a
complex and contested field [@Banister2008].
Therefore solutions to entrenched problems such as
car over-dependence cannot be found solely by reliance on numerical models:
policy, politics, leadership and vision are key ingredients for sustainable
urban mobility that computer models cannot provide [@Melia2015a].

A more specific limitation of the NPCT method is its lack of inclusion of
cycle route quality. Recent advances in cycle path network measurement
[@Joo2013]
could allow route network data to be fed-into more sophisticated versions
of the tool. However, we would argue that cycle path quality is not something
that needs to be modelled at the meso-level in a tool such as the NPCT. The
tool's niche is strategic planning for cycling uptake: once areas, travel
corridors and road segments have been prioritised by the NPCT, it is up to
local transport engineers to ensure that the quality of the new paths is sufficient.

In the realm of geographically specific strategic transport planning
the NPCT can be seen not only as a tool but as a framework.
...
Where applicable,
these are discussed in relation to future research priorities. Indeed,
one of the major motivations for writing this paper is to showcase the method
...
greatest. By indicating where investment could be most cost-effective,
the NPCT methodology and interactive planning tool will also help to
build business cases for further investment and policy change.
Careful allocation of resources is critical for creating
highly cost-effective interventions, with benefit-cost ratios
in double figures. Such targetted interventions are
critical to ensure a long-term transition away from the private car and
towards a more sustainable and healthy transport system.

# Acknowledgements:

We would like to thank the following people for comments on earlier versions of the report and the direction of the flow-level model:
Roger Geffen, Ian Philips and John Parkin.
