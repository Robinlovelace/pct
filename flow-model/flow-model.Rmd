...
The Propensity to Cycle Tool (PCT) seeks to inform such decisions by providing evidence-based support tools that model current and potential future
distributions in cycling across cities and regions. This paper
...
'Government Target' scenario, to double cycling. A more ambitious
...
would otherwise be totally dependent on motorised modes or
other people [@Aldred2008].
Mounting evidence external costs associated with car-dominated
...
[@Geertman2009]. As with such previously documented online systems, the PCT
provides a range of information to the user to enable evidence-based policy
[@Pettit2013]. The PCT differs from previous work, however:
...
The open approach to model development is key to the utility of the PCT,
enabling it to be readily recreated by others in new contexts.
...
various scenarios for cycling futures at the city to national levels.
...
level active travel planning [@Moniruzzaman2012]. 
...
By aggregating data from the flow level to geographic zones, the PCT
...
a multiple regression model to estimate levels of commuter cycling at
...
The model underlying the PCT is based on flow data
...
This means that the
output of the PCT is simply a visualisation of the model's pre-calculated
...
the PCT. Advanced users are free to analyse and
and modify the PCT's underlying code base, however,
...
that would allow tools such as the PCT to estimate where future demand is greatest.
...
features of the urban environment that affect cycling, such as
...
this was used for the PCT model as the aim is to model population flows.
...
primarily means local Highway Authorities, which
...
The regions used as the building blocks of the PCT could adapt to
reflect new funding structures.
...
flows and polygons, representing administrative zones) for the PCT. Yet the
greatest value of the PCT lies in its ability to represent different
...
An exciting future direction of this research would
...
The PCT allows different scenarios (e.g. generated by participatory modelling)
...
- Cycling Delivery Plan (*cdp*). Under this scenario 
the level of cycling doubles. The *cdp* scenario represents
a short-term vision for growing cycling.
This scenario is closely tied to current propensities to cycle by age and gender, while other scenarios focus more on potential (based on trip characteristics).

- Gender Equality (*gendereq*). This scenario assumes that in each flow the
proportion of females cycling becomes the same as the proportion of males.
...
would result from gender equality being reached in England.

- Go Dutch (*dutch*).
While *cdp* and *gendereq* are based on current cycling behaviour,
the *dutch* scenario focuses on long-term potential.
If English people had the same likelihood of cycling given trips
(i.e. of the same distance and hilliness) as do the Dutch
...
importance (in the UK context), the factors implemented in PCT were:
...
associated with this flow is 1.6 km, resulting in cirquity values of
1.44 and 1.75 for 'fast' and 'quiet' routes respectively.
...
translate national targets into national aspirations.
...
The wide range of opportunities to extend the tool do not mean that the PCT
...
A more specific limitation of the PCT method is its lack of inclusion of
...
the PCT can be seen not only as a tool but as a framework.
...
behaviour, the PCT methodology can empower transport decision-makers across
...
the PCT methodology and interactive planning tool will help to
