---
title:
  "The National Propensity to Cycle Tool: A generalisable flow-level model of
...
route level in response to acheiving national, regional or city-wide targets.
...
across England. However,
as we shall see, the tool has much wider potential applications.
...
Because of the public funding, the focussed on the English context: a
car-dependent nation with high potential for growth in cycling.
(Note that many nations fit this description.)
The methodology could be used to explore scenarios for active travel overall
(walking and cycling) and the tool is extensible in many directions.
In some ways it is worth thinking of the NPCT as a framework for developing
interactive transport planning support tools for policy evaluation, the
present case study of cycling in England being just one of many potential
applications.
...
and embedded problem as the unsustainability of current travel patterns.
...
It is important to note that the models do not include the effects on
cycling rates of specific policies, such as a particular piece of new
infrastructure (although further work could explore adding such
capability). Instead, the models allow  estimation of where new cycling
...
therfore a high proportion of commuters can be represented by
...
15% of all possible orgin destination pairs but inclues almost 70% of commuters.
...
due to how the data was strutured.


## Zone Buffer

As previously discussed selecting a Local Authority 'LAD' region means
aribitraially ignoring all the MSOAs outside this region. If the region is a
self-containted hub this does not present a major issue but if the region is part
of a larger metripolitan district then there may be significat travel between zones
inside the LAD and ones that have been cliped outside.
...
and to give futher information to users.
...
- The addition of additional purposes of trips in the model.
An 'education layer' would be our recommendation to prioritise
in this area, based on the location of schools, and data
on travel to school patterns from the Department of Education.
(Note we would need to formally request data for this.)
This could inform the construction of
off-road 'cycle to school' networks.
