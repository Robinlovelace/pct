...
transport planning support tools to ensure that cycling investment is spent effectively.
...
simulate what the rate of cycling could be along the desire lines
route network in the study region under various future scenarios.
...
the PCT. Advanced users can modify any part of the model.
...
skewed: a small number of OD pairs along major travel corridors
account fo a disproportionately high proportion of travel.
In the City of Manchester, for example,
setting `mflow` to 30 reduced the number of OD pairs by 85%
still accounted almost 70% of commuters.
...
ignoring all the MSOAs outside this region. If the region is a
self-contained transport system this does will not create problems.
But if the region is part
of a larger conurbation the clipping could be problematic, as
travel between zones inside the LAD and those directly outside would be
igored.
To solve this problem for smaller regions we created buffers around each region,
from which additional zones were sampled in a process that was triggered if
the number of zones fell below some threshold.
...
cycles ('E-bikes').
E-bikes enable longer journeys, as well as making cycling a viable option for older
