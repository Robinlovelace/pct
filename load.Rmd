...
mzarea <- median(gArea(zones, byid = T) / 1000000) # average area of zones, sq km
```

- Number of zones: `r nzones`, compared with 6791 in England
- Median area of zones: `r mzarea` ha, compared with 3 ha in the UK
...
```{r}
qtm(zones, "avslope", fill.palette = "Blues")
```
...
The most computationally challenging part of the process is allocating the 
flows to the travel network. We do this using CycleStreets.net, who
estimate optimal routes as part of a journey planning application created
for cyclists by cyclists. An illustration of these routes is presented below.
...
```{r plot-rlines, fig.cap="Sample of the straight and route-lines allocated to the travel network"}
plot(l[1:300,])
...
```{r, fig.cap="Distance distribution of all trips in study lines (blue) compared with national average (dotted bars)"}
l1 <- rep(l$dist_fast, l$All) 
luk <- readRDS("pct-bigdata/national/l_sam8.Rds")
l2 <- rep(luk$dist_fast, luk@data$All) 
# hist(l1, freq = T)
# hist(l2, freq = T)
ggplot() +
  geom_histogram(aes(x = l$dist_fast, weight = l$All / sum(l$All)),
    binwidth = 0.5, fill = "blue") +
  geom_histogram(aes(luk$dist_fast, weight = luk$All / sum(luk$All)),
...
pl5kmuk <- round(length(l2[l2 < 5]) / length(l2) * 100, 2)
pl5km <- round(length(l1[l1 < 5]) / length(l1) * 100, 2)
```

Nationwide, `r pl5kmuk`% of trips are less than 5km; in the case study area
`r pl5km`% of trips are less than 5km. Subsetting by distance (set
...
dfcos <- rbind(coef(mod_nat), coef(mod_logsqr))
```
