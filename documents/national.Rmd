...
in a city or region.
Regression analysis of 2001 census travel to work data
at the ward level (average population of around 6,500 people)
has demonstrated that the rate of cycling to work
can be explained by some of these variables [@Parkin2008]. 
The subsequent analysis investigates the current rate and future potential
of cycling in England based on 2011 census data on mode of travel
to work by distance bands (Parkin's work only considered modal split).

It is worth considering at the outset the strengths and limitations of this
data source. The main advantage is that census data is very comprehensive (with
...
nationally and make up a much higher proportion of rush hour traffic, where
...
for example, the majority of commuting trips are
...
of cycling. At the national level cycling has grown in recent years
...
Estimates of plausible future rates^[By
...
building on the momentum behind cycling in 'early adopter' areas.
It has been suggested that "towns with a pre-existing culture of cycling
find it easier to get more people  cycling because they do not have to overcome conflicting social norms" [@Sloman2014]. On the other hand, the greatest benefits may be in
...
The region of analysis is the entirety of England and is based on
2011 commuting data. As mentioned,
this dataset has the greatest coverage (90% + of the working population),
geographical resolution and coverage (with a high response rate from
every authority) of any travel dataset of England. Cycle commuting is
an important component of utility cycling, which has the great benefits in
terms of health (for it is integrated into everyday life) and
congestion (for it can reduce rush hour car traffic).
The analysis does not seek to estimate the current or potential level of
leisure cycling.
The potential of utility cycling is also central
the National Propensity to Cycle Tool project, of which
...
are provided Table 1. Note that the variable '% v. short car'
refers to the % of commutes below 2 km or less that are made by car,
reflecting the extent to which cars dominate local travel in the area.

Table: The top 10 'high cycling' Local Authorities in terms of the
proportion of commutes made by bicycle from the 2011 Census. See
...
t0 <- select(t0, Name = NAME, `Current %` = pcycle, `Expected %` = pcycle_exp, `Current (n)` = Bicycle_All, `Expected (n)` = expected, `% v. short car` = shortcar)
...
t0.1 <- select(t0.1, Name = NAME, `Current %` = pcycle, `Expected %` = pcycle_exp, `Current (n)` = Bicycle_All, `Expected (n)` = expected, `% v. short car` = shortcar)
...
cycling is unlikely to become the dominant mode of transport in the short term
in areas where relatively few short-distance trips take place.
(In the longer term, however, increased levels of local economic activity and
community cohesion could lead cycling to become the dominant mode even in
current 'commuter belts'.)
...
of commuting trips are 'short' yet the rate of commuter cycling is high at
...
advantage of promoting cycling (e.g. via new high quality cycle paths,
cycle parking and disincentives to drive) in an area that already
...
The advantage of estimating the expected rate of commuter cycling in this
...
presented in Table xx. See [github.com/npct/pct-data/blob/master/las_data.csv](https://github.com/npct/pct-data/blob/master/las_data.csv) for the complete .csv dataset.

Table: Top 10 Local Authority Districts (LADs) in terms of potential growth in
...
t0.1 <- select(t0.1, Name = NAME, `Current %` = pcycle, `Growth %` = pcycle_exp, `Current (n)` = Bicycle_All, `Growth (n)` = Bicycle_exp, `% v. short car` = shortcar)
t0.1 <- t0.1[10:1,]
kable(t0.1, digits = 2, row.names = F)

# save las data
lasdat_df <- select(las@data, Name = NAME, `N. commuters (all modes)` = All_All, `Current %` = pcycle, `Expected %` = pcycle_exp, `Current (n)` = Bicycle_All, `Expected (n)` = expected, `% v. short car` = shortcar, `Short trips (%)`)
...
```{r, echo=FALSE, fig.cap="Top 10 areas in terms of potential growth in cycling at the level of highway authorities in England. The full dataset can be viewed online: github.com/npct/pct-data/blob/master/cuas_dat.csv", message=FALSE}
# Do for cuas too?
library(rgdal)
...
cuasdat <- select(cuas@data, Name = CTYUA12NM, `All commutes (n)` = All_All, `Growth in cycling (n)` = Bicycle_exp)
summary(cuasdat)
...
5 km, to have a very high rate of cycling *all the way to work*.^[Integrating cycling with public transport in such areas, e.g. by
