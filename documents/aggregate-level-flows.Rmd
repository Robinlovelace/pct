...
plot(gflow$mdist, midpoint, xlab = "Mean distance (km)")
abline(a = 0, b = 1)
```
...
# lines(gflow$mdist, exp(modw_logcub$fitted.values), col = "green", lty = 2, lwd = 6)
```
...
nls_5pd1 <- nls(mbike ~ r * s * 10^(r * (mdist)) * (10^(r * (mdist)) + 1)^(-s-1), data = gflow, start = list(r = 0.1, s = 1))

nls_5pd2 <- nls(mbike ~ r * s * 10^(r * (mdist)) * (10^(r * (mdist)) + 1)^(-s-1), data = gflow, start = list(r = 0.2, s = 1), weights = total)

# nls_5pd3 <- nls(mbike ~ r * s * 10^(r * (mdist * xmid)) * (10^(r * (mdist * xmid)) + 1)^(-s-1), data = gflow, start = list(xmid = 0.8, r = 0.2, s = 1), weights = total) 

nls_5pd4 <- nls(mbike ~ r * s * 10^(r * (mdist)^t) * (10^(r * (mdist)) + 1)^(-s-1), data = gflow, start = list(t = 1, r = 0.2, s = 1), weights = total) 

nls_5pd5 <- nls(mbike ~ r * s * 10^(r * (mdist)) * (10^(r * (mdist)) + 1)^(-s-1) + t, data = gflow, start = list(t = 1, r = 0.2, s = 1), weights = total) 

ynl1 <- predict(nls_5pd1, newdata = data.frame(mdist = x)) 
ynl2 <- predict(nls_5pd2, newdata = data.frame(mdist = x)) 
ynl3 <- predict(nls_5pd3, newdata = data.frame(mdist = x)) 
ynl4 <- predict(nls_5pd4, newdata = data.frame(mdist = x)) 
ynl5 <- predict(nls_5pd5, newdata = data.frame(mdist = x)) 
...
# lines(df$dist, predict(object = mod_ind2, df), col = "red", lwd = 2, lty = 2)
lines(gflow$mdist, exp(modw_logcub$fitted.values), col = "green", lty = 2, lwd = 3)
```
