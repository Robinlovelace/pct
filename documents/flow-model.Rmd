...
The model is open and transparent,
based on input data that
is widely available in many countries around the world.
The model is implemented in R, an open-source language for
...
Although the original model was developed for
England, it has the
potential to assist with the design of transport policy worldwide.
...
associated investment to be spent optimally.
...
The model operates at the flow-level. 
A 'flow' in this context constitutes an
origin-destination pair: the place where the trip begins and
a destination where it ends.
The model takes the current rate of cycling and other variables per flow
and uses this to project the future rate of cycling
per flow under various scenarios.
These desire lines connect residential
origins with workplace, education or other destinations. It is important to
note that the model is not intended to be
predictive and does not assume the implementation of
any specific policies (although further work on model could explore adding such
capability). The model operates 'under the hood' in the NPCT, meaning that the
...
through the project's repository if they would like to change any of the
model's internals or add new features.^[See
[github.com/npct/pct](https://github.com/npct/pct).]

A 'model output' tab was added to the
user interface, which served 3 purposes: to avoid 'hiding' the
underlying model from users, encourage modifications to
