---
title: Where to build bike paths? A generalisable flow-level model of cycling uptake for sustainable transport planning
...
The National Propensity to Cycle Tool (NPCT) is an interactive policy support
application designed to inform the decision-making process around where to
invest in new sustainable transport infrastructure. Funded by the Department
for Transport (DfT), the primary purpose of the NPCT
is to be deployed nationwide and used by
transport planners to help allocate new investment in cycling. The NPCT is
based on open source software and requires input data that is widely available
in many countries around the world. It therefore has great potential to assist
with the design of transport policy worldwide, as part of a global transition
away from fossil fuels that is required to prevent runaway climate change.
The worldwide uptake of cycling, driven by well-placed infrastructure
(the design of which can be assisted by tools such as the NPCT), 'soft' policy
incentives and wider cultural shifts away from the car will also have
enormous health benefits. 

The aim of the present paper is to explain the underlying model which drives
the scenarios used in the NPCT. The flow-level model (which operates at
the level of origin-destination pairs over the study area) projects what
the rate of cycling *could* be along travel 'desire lines' connecting
residential origins with workplace, education or other destinations.
It is important to note that the model is not predictive and does not
assume the implementation of any specific policies (although further
work on model could explore adding such capability). The model operates
'under the hood' in the NPCT, meaning that the output of the NPCT is simply
a visualisation of the model that will be useful for transport modellers,
rather that a summary of the model overall.^[However, a 'model output' tab
in the model...]
...
breakdown in the geographic distribution of cycling uptake in
the scenario, however, making it difficult for transport planners
to know where new demand is likely to be greatest.
