...
focusses on Phase I of the work. In 'Phase II' we will extend to the model
...
associated investment to be spent effectively, yielding very high returns on
...
regions. Based on these insights, and feedback from practitioners, our
...
The demarcation of regional boundaries is deemed useful for focussing
...
For our NPCT phase 1 model, the route allocation algorithm, implemented 'in the
cloud' by CycleStreets.net (an online routing service for cyclists, by cyclists),
...
We set this threshold to 60 for Manchester. Because there are 57 MSOA
zones in Manchester, this led to the selection of additional zones
outside the long and thin Manchester LAD.
...
underlying code. This is thanks to the 'Model Output' tab (Fig. 3).
...
To estimate the number of cyclists at the flow level, a regression model
was used. The optimisation seeks to modify a number of model parameters that
link the *explanatory variables* described above to the *dependent variable*:
the proportion of trips made by bicycle per flow ($pcycle$). Central to this
model is *distance decay*, which describes the (non-linear) relationship
between the distance of a trip and the probability of it being made by bicycle.
After trying various functional forms, we found that the distance decay
curve displayed in Fig. 4, and variations thereof, involving changing
the parameters. 

```{r, echo=FALSE, fig.cap="The distance decay curve resulting from the log-square-root function. The green lines represent increases of 0.25 times the the original values and the red lines represent -0.1 times the original value."}
...
$$
log(pcycle) \approx \frac{exp(\alpha + \beta_1 X1 + \beta_2 X1 + \beta3 X2)}{exp(1 + \alpha + \beta_1 X1 + \beta_2 X1 + \beta3 X2)}
$$

where $X1$ is distance (km, route distance), $X2$ is the hilliness per flow and
...
that is possible due the electric bicycles (hence the scenario name, *ebike*).
This is the most ambitious of the scenarios presented in this paper and it
builds on 'Go Dutch', but starting with Dutch model parameters. 

To ensure that the scenario represents the impact on longer trips,
it is the first (linear) $\beta$ parameter that is increased by a small amount.
In Phase I we increased this value by 0.025 a value sufficiently small to avoid
generating an implausibly high rate of cycling but sufficiently large to create
a noticeable effect.
In Phase II we will base the changes to the distance decay parameters on
real data from the Dutch National Travel Survey.

# Results

To demonstrate
how the scenarios work in practice and to provide an overview of the results,
Fig. 6 illustrates the observed ($OLC$, from the
2011 Census) and scenario-based levels of cycling
...
trips that are made by bicycle is lower. There is noticeable distance decay
of for all modes of transport, especially for bicycle trips in Norwich,
where bicycle trips above 7.5 km observed from 2011 Census data are
comparatively rare.
...
and overiding the initial rate of cycling. Note also that the *cpd* scenario
in Manchester has a considerably higher rate of cycling than the *gendereq*
scenario, wheras in Norwich these scenarios are very similar. This is because
...
wheras under *dutch* assumptions, the distribution shifts to flows that
are more representative of short-distance flows in across the city overall.
In both cases the flows are focussed around Norwich city centre: the region
has a monocentric regional economy, making trips beyond around 5 km from the
centre much less likely to be made by bicycle.
...
Manchester has a polycentric structure, favouring the construction of bicycle
paths between the various sub-centres, not just in radial routes to a single
...
along Trinity Way (the A6042), a busy dual carraige way. This suggests
that new cycle infrastructure along this route would be well-used by
commuters.
...
allocation of investment and local targetting of policies within
...
- International and inter-regional comparisons of model results. This could
...
off-road 'cycle to school' network to enable children healthy, economic and
independent means for undertaking the daily 'school commute'. This would
also have a large financial saving by eliminating thousands of adult hours
stuck in traffic.

Because the model is open source, others are free to take the tool and
modify it for their own needs. We actively encourage practitioners to modify
the scenarios, input data and display of the results to suit their local needs,
which will vary considerably from place to place. It is hoped that an active
user community is built-up around the tool, enabling transport planners to
decide the precise contents of the software that are most useful for them.
It is likely that many of the new features added by the community have not
been considered in this paper. After all, the people who best understand the
...
creation and visualisation of evidence-based scenarios of change in travel
