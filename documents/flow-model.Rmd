...
an interactive map-based policy support application for
developing transport policies, funded by the UK's Department
for Transport (DfT). The NPCT will was originally designed for
use by Local Authority transport planners
to prioritise investment in cycling in England. However,
as we shall see, the tool has much wider potential applications.
Funded by central government to assist local transport planning, the
tool's main purpose
is to help prioritise where to build new strategic cycle routes
and related infrastructure at city and regional levels.
...
data quality. Official datasets are available for commuting trips and in some cases education trips.
...
is to create buffers around the the regions, which we have already
implemented in the prototype for Phase I (as discussed below).
We plan to test each of these options in Phase II, and ask
...
is the major bottle-neck at present in terms of time required to run the model
for a given region.  In order to reduce the time to add new regions
...
In Phase II this will be solved by using larger regional units.
For Phase I we created buffers around each region,
...
The complete model output document for Manchester is provided
in Appendix X.
...
was conducted as part of the Co-benefits model. See Appendix.xxx.
...
- Inter-regional and international comparisons of model results
(see Appendix xx). This could
