...
under various scenarios about the future. 
...
level of cycling. The NPCT model translates this headline figure into
...
More specifically: along which routes would the new cycle trips occur?
...
explore using Lower
Super Output Areas (LSOAs, average population of around 1,600).
The centroid data can potentially be derived from boundary data,
by finding the geographic centroid. If available, alternatives such
as the population-weighted centroids may be more informative
--- in England,
the population-weighted centroids were provided as a publicly
...
We plan to implement these recommendations in Phase II.
...
of over 20km. This distance is not feasible for most people to cycle on a daily basis.
...
In line with the target set out in the Cycling Delivery Plan, the *cdp* scenario
...
distance of the flow nationally. The new percent cycling
...
cycle based on the national-level regression model. The sum of these values
is multiplied by $tflow$, the total number of commuters, by all modes,
in order to convert the proportion into
a number of cyclists.

```{r, echo=FALSE}
# add stuff here! (from AG)
```
...
and overriding the initial rate of cycling. Note also that the *cpd* scenario
...
Manchester has a poly-centric structure, favouring the construction of bicycle
...
along Trinity Way (the A6042), a busy dual carriage way. This suggests
